...
void main()
{
	...
	float height = texture(u_image, uv).r;
	float heightScale = 10.0f;

	float right = textureOffset(u_image, uv, ivec2(1,0)).r * (heightScale * 8.0f);
	float left = textureOffset(u_image, uv, ivec2(-1,0)).r * (heightScale * 8.0f);
	float up = textureOffset(u_image, uv, ivec2(0,1)).r * (heightScale * 8.0f);
	float down = textureOffset(u_image, uv, ivec2(0,-1)).r * (heightScale * 8.0f);

	float lr = left - right;
	float du = down - up;
	vec4 tes_CDMnormal = vec4(normalize(vec3(lr, 2.0, du)), 1.0);
